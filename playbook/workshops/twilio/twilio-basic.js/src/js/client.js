...
function twilioCallback(twilio) {
  ...
  Twilio.sendMessage = function(number, message, callback) {
    Twilio._runWhenInitialized(function() {
      ...
      client.messages.create({
        ...
        if(err === null) {
          Twilio.log("Sucessfully sent message '{0}' to '{1}'.", message, number);
          if (callback) {
            callback(response);
          }
        }
        else {
          Twilio.log("ERROR: Could not sent message '{0}' to number '{1}' because '{2}'", message, number, err.message);
          if (callback) {
            callback(err);
          }
        }
  ...
  Twilio.callAndSay = function(number, message, callback) {
    Twilio._runWhenInitialized(function() {
      ...
      var twimletUrl = "http://twimlets.com/message?Message%5B0%5D=" + encodedMessage;
      client.makeCall({
        to: number,
        from: Twilio.PHONE_NUMBER,
        url: twimletUrl
        ...
        if(err === null) {
          Twilio.log("Sucessfully called number '{0}' and about to say '{1}'", number, message);
          if (callback) {
            callback(response);
          }
        }
        else {
          Twilio.log("ERROR: Could not call number '{0}' and say '{1}' because {2}", number, message, err.message);
          if (callback) {
            callback(err);
          }
        }
  ...
  Twilio.callAndPlay = function(number, mp3Url, callback) {9
    Twilio._runWhenInitialized(function() {
      Twilio.log("Calling '{0}' and will play '{1}'...", number, mp3Url);
      var encodedMp3Url = encodeURIComponent(mp3Url);
      var twimletUrl = "http://twimlets.com/message?Message%5B0%5D=" + encodedMp3Url;
      client.makeCall({
        ...
        if(err === null) {
          Twilio.log("Sucessfully called number '{0}' and about to play the music file '{1}'", number, mp3Url);
          if (callback) {
            callback(response);
          }
        }
        else {
          Twilio.log("ERROR: Could not call number '{0}' and play the music file '{1}' because {2}", number, mp3Url, err.message);
          if (callback) {
            callback(err);
          }
        }
  ...
  Twilio.getLatestMessage = function(callback) {
    Twilio._runWhenInitialized(function() {
      dweetio.get_latest_dweet_for(Twilio._getStrippedNumber(), function(err, dweets) {
  ...
  Twilio.getAllMessages = function(callback) {
    Twilio._runWhenInitialized(function() {
      dweetio.get_all_dweets_for(Twilio._getStrippedNumber(), function(err, dweets) {
  ...
  Twilio._provisionNewNumber = function(callback) {
    ...
    client.availablePhoneNumbers("US").local.list(phoneRequirements, function(err, numbers) {
      ...
      client.incomingPhoneNumbers.create({
        ...
        console.log("Your Twilio phone number is " + purchasedNumber.phoneNumber);
