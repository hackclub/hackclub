...
Now nest two more divisions in `<div id="box" class="box">` with class as “box1” and “box2” respectively. Just place both of these in between that `<div ...>` and `</div>`.
...
We have created all the options, now we will just add the `spin` button which we saw in the center. For that just write (`<button class="spin" onclick="myfunction()">SPIN</button>`) before the ending of your last division tag. Here we have used the onclick function that will be explained in the JavaScript part of our workshop.  
```html
<html>
  <head>
...
## 2)	CSS
After you have done the writing part of HTML, you'll notice that when you click the "run" button at the top, you just see a white screen with some text("options") with Spin Button. Let's now add some styles to our elements and make them look better like the way we want it to be.🤩
...
Now in the next block, we have `font-family: Open Sans;` this syntax is used to give the name/type of font we want to have in our block. Here we have used it in selector as body therefore it will be applied as a default font on our whole webpage.
...
In the background section, you can use any color’s hex code, but in this workshop, I have used gradient which you may get from the [CSS gradient](https://cssgradient.io/). Go on that website and make your gradient then copy the code given below and paste it in the background section. By this, your custom background will be applied.
