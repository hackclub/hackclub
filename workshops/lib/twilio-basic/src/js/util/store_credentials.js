...
while (sid === null || sid.length !== 34) {
  sid = prompt(
    'You mistyped your Twilio ACCOUNT SID. Please retype it.' +
...
while (token === null || token.length !== 32) {
  token = prompt(
    'You mistyped your Twilio AUTH TOKEN. Please retype it.' +
      ' For help, check https://github.com/hackclub/hackclub/tree/master/playbook/workshops/lib/twilio-basic/README.md'
  )
}
...
setTimeout(function () {
  if (Twilio.INITIALIZED === false) {
    ...
    alert(
      'Your Twilio ACCOUNT SID and Twilio Token is invalid' +
